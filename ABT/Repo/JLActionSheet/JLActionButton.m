...
+ (id) buttonWithStyle:(JLActionSheetStyle*)style andTitle:(NSString *)buttonTitle isCancel:(BOOL) isCancel
{
    ...
    if (!isCancel)
    {
        if ([buttonTitle isEqualToString:@"Kan"]) {
            actionButton.bgColor = [UIColor greenColor];
            actionButton.highlightedColor = [UIColor blackColor];
        }
        else if ([buttonTitle isEqualToString:@"Helst ikke"]){
            actionButton.bgColor = [UIColor orangeColor];
            actionButton.highlightedColor = [UIColor blackColor];
        }
        else if ([buttonTitle isEqualToString:@"Kan ikke"]){
            actionButton.bgColor = [UIColor redColor];
            actionButton.highlightedColor = [UIColor blackColor];
        }
        else{
    ...
    [actionButton.titleLabel setFont:[UIFont fontWithName:LIGHT size:18]];
